...
variable "cloudflare_pages_project_name" {
  description = "Cloudflare Pages プロジェクト名（例: devforge-stg）。"
  type        = string
  default     = "devforge-stg"
}

variable "cloudflare_subdomain" {
  description = "Cloudflare DNS レコード名（例: app-stg）。{subdomain}.devforge.app となる。"
  type        = string
  default     = "app-stg"
}
...
variable "turso_database_url" {
  description = "Turso (libSQL) データベースの接続 URL（例: libsql://devforge-stg-xxxx.turso.io）。turso CLI で発行する。GitHub Secret → TF_VAR_turso_database_url で注入する。"
  type        = string
}
