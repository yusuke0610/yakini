...
- Cloud Run は `/tmp/yakini.sqlite` を使用（`SQLITE_DB_PATH` 環境変数）
...
```
SQLITE_DB_PATH       # Cloud Run: /tmp/yakini.sqlite
...
GCS_DB_OBJECT        # 例: yakini/dev/db.sqlite
ADMIN_TOKEN          # /admin/backup エンドポイント用
CORS_ORIGINS         # 例: https://yakini-dev.example.com
```
...
terraform {
  ...
  backend "gcs" {
    bucket = "yakini-tfstate-{env}"   # 環境別バケット
