...
def _latest_rows(conn, table_name: str) -> list[dict]:
    rows = conn.execute(sa.text(
        f"SELECT * FROM {table_name} ORDER BY updated_at DESC, created_at DESC"
    )).mappings()
...
def _create_normalized_tables() -> None:
    ...
    op.create_index("ix_rirekisho_educations_rirekisho_id", "rirekisho_educations", ["rirekisho_id"])
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("account_id", "external_id", name="uq_blog_articles_account_external_id"),
    )
...
def upgrade() -> None:
    ...
    article_rows = conn.execute(sa.text(
        """
