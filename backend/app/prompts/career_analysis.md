...
### GitHub 個人活動の評価（フレームワーク・設計思想）
- 「主要言語」は言語成熟度の目安として扱い、**言語割合だけで結論を出さない**こと
- 「検出フレームワーク」セクションに並ぶ値（React / Next.js / Django / FastAPI / Spring Boot 等）は
  依存ファイル（package.json / requirements.txt / pyproject.toml / pom.xml / go.mod）と
  ルートファイル（Dockerfile / CI 設定 / Terraform 等）から抽出された一次根拠である
- これらは**利用しているフレームワークと設計思想の証拠**として扱い、
  フロントエンド／バックエンド／インフラ等の強みを判定する際に優先的に参照すること
- 例: 「TypeScript が主要言語」であっても「検出フレームワーク」に `React` / `Next.js` が無ければ
  SPA 実装経験があると断定しない。逆に含まれていれば React 経験として評価する
- 「検出スキル」は言語・トピック・ファイルを横断して抽出した派生指標であり、
  根拠としては「検出フレームワーク」より弱いものとして扱うこと
